...
'''
ts_2_server() takes in no arguments and houses the logic behind the ts_2 server. Upon starting the TS server logic it will create a table which will
...
def ts_2_server():
    ...
    while True:
        ...
        if queried_hostname in table.keys():
            ...
            print("Sent: ",table[queried_hostname])

    ss.close()
    exit()

'''
This is the main method of ts.py where ts_2.py is expecting two arguments to start (python ts_2.py) and (the portnumber)
to start successfully.
'''
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Incorrect Usage: python ts_2.py <port number>")
        sys.exit(1)
    ts_2_server()